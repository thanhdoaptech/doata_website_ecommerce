            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('test') ? 'is-invalid' : '' }}" name="test_id" id="test_id" required>
                    @foreach($tests as $id => $test)
                        <option value="{{ $id }}" {{ ($testResult->test ? $testResult->test->id : old('test_id')) == $id ? 'selected' : '' }}>{{ $test }}</option>
                    @endforeach
                </select>
                @if($errors->has('test'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('student') ? 'is-invalid' : '' }}" name="student_id" id="student_id" required>
                    @foreach($students as $id => $student)
                        <option value="{{ $id }}" {{ ($testResult->student ? $testResult->student->id : old('student_id')) == $id ? 'selected' : '' }}>{{ $student }}</option>
                    @endforeach
                </select>
