            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('test_result') ? 'is-invalid' : '' }}" name="test_result_id" id="test_result_id" required>
                    @foreach($test_results as $id => $test_result)
                        <option value="{{ $id }}" {{ ($testAnswer->test_result ? $testAnswer->test_result->id : old('test_result_id')) == $id ? 'selected' : '' }}>{{ $test_result }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('question') ? 'is-invalid' : '' }}" name="question_id" id="question_id" required>
                    @foreach($questions as $id => $question)
                        <option value="{{ $id }}" {{ ($testAnswer->question ? $testAnswer->question->id : old('question_id')) == $id ? 'selected' : '' }}>{{ $question }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('option') ? 'is-invalid' : '' }}" name="option_id" id="option_id" required>
                    @foreach($options as $id => $option)
                        <option value="{{ $id }}" {{ ($testAnswer->option ? $testAnswer->option->id : old('option_id')) == $id ? 'selected' : '' }}>{{ $option }}</option>
                    @endforeach
                </select>
