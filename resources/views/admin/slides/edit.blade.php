    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.slides.update", [$slide->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('slide_types') ? 'is-invalid' : '' }}" name="slide_types[]" id="slide_types" multiple required>
                    @foreach($slide_types as $id => $slide_type)
                        <option value="{{ $id }}" {{ (in_array($id, old('slide_types', [])) || $slide->slide_types->contains($id)) ? 'selected' : '' }}>{{ $slide_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $slide->name) }}">
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $slide->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description_1') ? 'is-invalid' : '' }}" type="text" name="description_1" id="description_1" value="{{ old('description_1', $slide->description_1) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('button_name') ? 'is-invalid' : '' }}" type="text" name="button_name" id="button_name" value="{{ old('button_name', $slide->button_name) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ $slide->active || old('active', 0) === 1 ? 'checked' : '' }}>
