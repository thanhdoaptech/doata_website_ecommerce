...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-categories.update", [$productCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="category">{{ trans('cruds.productCategory.fields.category') }}</label>
                <input class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" type="number" name="category" id="category" value="{{ old('category', $productCategory->category) }}" step="1">
            ...
            <div class="form-group">
                <label class="required" for="sort">{{ trans('cruds.productCategory.fields.sort') }}</label>
                <input class="form-control {{ $errors->has('sort') ? 'is-invalid' : '' }}" type="number" name="sort" id="sort" value="{{ old('sort', $productCategory->sort) }}" step="1" required>
                @if($errors->has('sort'))
                    <div class="invalid-feedback">
                        {{ $errors->first('sort') }}
                    </div>
