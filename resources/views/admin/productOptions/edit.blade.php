...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.productOption.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-options.update", [$productOption->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}" name="products[]" id="products" multiple>
                    @foreach($products as $id => $product)
                        <option value="{{ $id }}" {{ (in_array($id, old('products', [])) || $productOption->products->contains($id)) ? 'selected' : '' }}>{{ $product }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('options') ? 'is-invalid' : '' }}" name="options[]" id="options" multiple>
                    @foreach($options as $id => $option)
                        <option value="{{ $id }}" {{ (in_array($id, old('options', [])) || $productOption->options->contains($id)) ? 'selected' : '' }}>{{ $option }}</option>
                    @endforeach
                </select>
